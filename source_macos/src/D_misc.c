 ...
 for (i=1;i<my_argc;i++)
  {
   ...
   if (!*parm++) break;   	// hit end of string without an alphanum
   ...
   shuts everything down */
{
 ...
 va_end(argptr);
 fprintf(stderr,"%s\n",buf);
 /* The Win32 version only called PostQuitMessage here and then RETURNED, so
    every caller carried on with whatever null pointer had triggered the
    error.  Terminate for real instead. */
 Sys_ErrorBox(buf);
 exit(1);
 }
